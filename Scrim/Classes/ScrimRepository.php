...
final class ScrimRepository
{
    ...
    public static function create(Player $admin, int $days, string $teamA, string $teamB, string $name): int
    {
        ...
        if (strcasecmp($teamA, $teamB) === 0 || trim($teamA) === '' || trim($teamB) === '') {
    ...
    {
        $scrim = self::current();
        if (!$scrim || $scrim->status !== ScrimState::ACTIVE || !$scrim->end_at || strtotime($scrim->end_at . ' UTC') > time()) {
            return false;
        }
    ...
    public static function addMap(Player $admin, string $uid, string $name): void
    {
        ...
        DB::table('scrim-maps')->updateOrInsert(['scrim_id' => $scrim->id, 'map_uid' => $uid], ['map_name' => $name ?: $uid]);
